...
class Chess:
    ...
    def highlight(self, y, x, p):
        ...
        if self.check_check:
            if piece == p:
                self.check_val = True
            elif ((p == 2 or p == 4) and piece == self.wQUEEN) or ((p == 12 or p == 14) and piece == self.bQUEEN): 
                self.check_val = True
            elif (p == 1 and piece == 11) or (p == 11 and piece == 1):
                self.check_check = True
            return None
        

        if p < 10:
            if piece == 0 or piece > 10:
    ...
    def evalutation(self, y, x):
        try:
            self.stockfish.make_moves_from_current_position([self.get_coords(y, x)])
            ev = self.stockfish.get_evaluation(searchtime=50)
            ev_type = ev['type']
            cp = ev['value']
